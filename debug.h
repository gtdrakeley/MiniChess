...
#define strerr()
#define log_error(S, ...)
#define log_error_nec(S, ...)
#define log_warning(S, ...)
#define log_info(S, ...)
#define verify(C, S, ...)
#define verify_nec(C, S, ...)
#define sentry(S, ...)
#define verify_mem(M, ...)
...
#define log_error_nec(S, ...) fprintf(stderr, "[ERROR] (%s:%d) " S "\n", __FILE__, __LINE__, ##__VA_ARGS__)
...
#define verify_nec(C, S, ...) if (!(C)) { log_error_nec(S, ##__VA_ARGS__); goto error; }
